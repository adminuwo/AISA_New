...
import { Search, ChevronDown, X, Landmark, Check } from 'lucide-react';
import { STATES_BY_COUNTRY } from '../data/statesData';
...
const StateSelect = ({
  ...
  // Load states/provinces for the selected country
    ...
    <div ref={containerRef} className="relative" onKeyDown={handleKeyDown}>
      ...
      <button
        ...
        className={`w-full flex items-center gap-2 border rounded-xl px-4 py-3 text-sm font-medium outline-none transition-all ${
          !country ? 'opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-white/5' : 'focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500'
        } ${borderClass} text-left`}
      >
        <Landmark size={15} className={`shrink-0 ${!country ? 'text-slate-300 dark:text-zinc-700' : 'text-slate-400'}`} />
        <span className={`flex-1 truncate ${value ? 'text-slate-800 dark:text-white font-semibold' : 'text-slate-400'} flex items-center gap-1.5`}>
          ...
          {value && country && <Check size={14} className="text-emerald-500 shrink-0" />}
        </span>
        {value && country && (
          <button
            type="button"
        ...
        >
          ...
          >
            {filtered.map((stateName, idx) => {
              ...
              return (
                ...
                >
                  <span className="text-xs font-semibold flex-1 truncate pr-2">{stateName}</span>
            ...
            {query.trim() && !states.some(s => s.toLowerCase() === query.trim().toLowerCase()) && (
              <button
                type="button"
                role="option"
            ...
            {filtered.length === 0 && !query.trim() && (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-xs font-bold text-slate-400">No states found. Type custom state above.</p>
              </div>
            )}
          </div>
          ...
          <div className="px-4 py-2 border-t border-slate-100 dark:border-white/5 bg-slate-50/80 dark:bg-black/10">
            <p className="text-[10px] text-slate-400 font-medium">
              {filtered.length} states/provinces loaded for {country}
            </p>
          </div>
        </div>
      )}
